...
function isInner(pos) {
    return pos.x > 2 && pos.x < grid[0].length - 3 && pos.y > 2 && pos.y < grid.length - 3;
...
function getPath(start, target) {
    ...
    while (queue.length > 0) {
        ...
        current = queue.shift();
        // console.log('checking',current, bestRoute.get(current));
        ...
        var neighbours = getNeighbours(current, bestRoute);
        ...
        for (var n of neighbours) {
            ...
            if (route < bestRoute.get(n)) {
                ...
                if (!queue.find(a => a.x == n.x && a.y == n.y && a.z == n.z)) {
                    queue.push(n);
                }
            }
        }
...
function getNeighbours(a, bestRoute) {
    var n = [];
    ...
    for (var portal in portals) {
        if (portals[portal].length > 1) {
            ...
            if (p) {
                ...
                if (inner && a.z < 50) {
                    toAdd.z = a.z + 1;
                    console.log('add p', toAdd, portal, 'from', a);
